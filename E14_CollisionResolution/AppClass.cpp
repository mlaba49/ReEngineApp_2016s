...
void AppClass::InitVariables(void)
{
	...
	m_pPalletL->SetModelMatrix(glm::translate(vector3(-9, 0, 0)));
	m_pPalletR->SetModelMatrix(glm::translate(vector3(9, 0, 0)));
}
...
void AppClass::Update(void)
{
	...
	if (m_pBall->IsColliding(m_pBoxR) || m_pBall->IsColliding(m_pBoxL) || m_pBall->IsColliding(m_pPalletL) || m_pBall->IsColliding(m_pPalletR))
	{
		vector3 v3Velocity = m_pBall->GetVelocity();
		v3Velocity.x *= -1;
		m_pBall->SetVelocity(v3Velocity);
